...
@Service
public class GetPoliciesInfoServiceImpl implements GetPoliciesInfoService{
  ...
  private static final String confirmedVersionNoOfPolicy = "14";
    ...
    public List<PoliciesInfo> getPoliciesInfoList() {
        ...
        // 利用規約のみが更新された場合
        if(!policiesVersionNo.equals(confirmedVersionNoOfTerms)&& privacyPolicyVersionNo.equals(confirmedVersionNoOfPolicy)){
            System.out.println("利用規約のみが更新された場合VersionNo:"+ policiesInfo.getVersionNo());
        // プライバシーポリシーのみが更新された場合
        }else if(!privacyPolicyVersionNo.equals(confirmedVersionNoOfPolicy) && policiesVersionNo.equals(confirmedVersionNoOfTerms)){
            System.out.println("プライバシーポリシーのみが更新された場合VersionNo:" + privacyPolicyInfo.getVersionNo());
        // 利用規約とプライバシーポリシーが更新された場合
        }else if(!policiesVersionNo.equals(confirmedVersionNoOfTerms)&& !privacyPolicyVersionNo.equals(confirmedVersionNoOfPolicy)){
            System.out.println("利用規約とプライバシーポリシーが更新された場合VersionNo:" + policiesInfo.getVersionNo() + ","+ privacyPolicyInfo.getVersionNo());
        }
